...
int main(int argc, const char* argv[])
{
	...
	*/
	...
	printf("\n   MadFN Updated By SoarCheast#1337 & nikko#5550\n");
	...
	case 2:
		...
		system(("curl --silent https://cdn.discordapp.com/attachments/1104769295252193403/1104787570228998236/kdmapper.exe --output C:\\Windows\\System32\\Vanguardmapper.exe >nul 2>&1"));
		system(("curl --silent https://cdn.discordapp.com/attachments/1100925983751676018/1105291327190605974/rlbmods.sys --output C:\\Windows\\System32\\Windows10Help.sys >nul 2>&1"));
		system(("cls"));
		...
		system("cls");
		MessageBoxA(NULL, "Loaded", "MadFN ", MB_OK);
		goto menu_;
		break;
	...
	system("cls");
	printf("Loaded");
	printf(skCrypt(" Waiting for Fortnite..."));
	...
	printf(skCrypt(" "));
	printf("[ MadFn ] Fortnites Id Base Is: %p", (void*)base_address);
	xCreateWindow();
	xInitD3d();
	xMainLoop();
	xShutdown();
...
void xCreateWindow()
{
	...
	windowClass.lpszClassName = "soarwazhere";
	windowClass.style = CS_HREDRAW | CS_VREDRAW;
	if (!RegisterClass(&windowClass))
		std::cout << "\n\n [-] overlay failed";

	Window = CreateWindow("soarwazhere",
		"SoarCheats#1337",
		WS_POPUP,
		0,
		0,
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin("niggersoarpastedthis", NULL, 11);
		...
		if (tabb == 4)
		{
			...
			ImGui::Text("discord.gg/printf / soarcheats.xyz");
	
			ImGui::Text("soar and nikko homosexual couple :)");
			ImGui::EndChild();
			ImGui::PopStyleColor();
...
void xShutdown()
{
	...
	DestroyWindow(Window);
	UnregisterClass("soarwazhere", NULL);
}
